...
df_hourly_valence_average_jonathan = pd.read_csv("data/hourlyStreamingIndividual/dfHourlyValenceAverageJonathan.csv")

# Extract values from df_hourly_valence_avereage
values = df_hourly_valence_average_jonathan['averageValenceScore'].tolist()
...
base = alt.Chart(source).encode(
    ...
    color=alt.Color("values:Q", legend=alt.Legend(title="Valence Score"), scale=alt.Scale(scheme="purples"))
)
...
radial_chart_average_valence_jonathan = (c1 + c2).properties(width=400, height=400)